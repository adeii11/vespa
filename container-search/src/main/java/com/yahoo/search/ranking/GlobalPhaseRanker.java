...
public class GlobalPhaseRanker {
    ...
    static GlobalFunEvalCtx withQueryPrep(GlobalFunEvalCtx evalCtx, Query query) {
        ...
        return new GlobalFunEvalCtx(supplier, List.of(), evalCtx.fromMF());
    }

    public void rerankHits(Query query, Result result, String schema) {
        var setup = globalPhaseSetupFor(query, schema).orElse(null);
        if (setup == null) return;
        GlobalFunEvalCtx mainCtx = withQueryPrep(setup.globalPhaseEvalCtx, query);
        int rerankCount = setup.rerankCount;
        var normalizers = new ArrayList<NormalizerContext>();
        for (var nSetup : setup.normalizers) {
            var normEvalCtx = withQueryPrep(nSetup.evalCtx(), query);
            normalizers.add(new NormalizerContext(nSetup.name(), nSetup.supplier().get(), normEvalCtx));
        }
        var rescorer = new HitRescorer(mainCtx, normalizers);
